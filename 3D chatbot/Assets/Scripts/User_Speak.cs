...
namespace Voxon
{
    public class User_Speak : MonoBehaviour, IDrawable
    {
        point3d _pr, _pd, _pp;
        ...
        public Vector3 pp = new Vector3(-0.75f, -0.5f, -0.2f);
        ...
        public void Update()
        {
            if (stt != null)
            {
                ...
                text = "User: " + stt.GetResult();

                if (stt.GetResult().Contains("hi 5"))
                {
                    text = "User: High-Five!";
                }
                
                Debug.Log("chat: " + stt.GetResult());
            }
            else
            {
