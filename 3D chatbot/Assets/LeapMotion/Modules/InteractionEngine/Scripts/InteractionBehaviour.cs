...
namespace Leap.Unity.Interaction {
  ...
  public class InteractionBehaviour : MonoBehaviour, IInteractionBehaviour {
    ...
    public void StayHovered(List<InteractionController> controllers) {
            ...
            foreach (var IC in controllers) {
                Debug.Log($"{IC}: {IC.isPrimaryHovering}");
                if (IC.isPrimaryHovering && IC.primaryHoverPoints.Count == 5) {
                    foreach (Transform point in IC.primaryHoverPoints) { 
                        if(point) Debug.Log($"POINT: {point.position}");
                     }

                    if (IC.primaryHoverPoints[0]) this.rigidbody.position = IC.primaryHoverPoints[0].position;
                    
                    /*
                    Vector3 newPos = new Vector3(Mathf.Clamp(IC.primaryHoverPoints[0].position.x, -0.001211f, 0.005621f),
                                                    Mathf.Clamp(IC.primaryHoverPoints[0].position.y, -0.005754f, 0.002004f),
                                                    Mathf.Clamp(IC.primaryHoverPoints[0].position.z, -0.1544f, -0.14713f));
                    
                    

                    if (IC.primaryHoverPoints[0]) this.rigidbody.position = newPos;
                    */
            ...
            // Debug.Log($"{this.rigidbody.name} Pos: {this.rigidbody.position}");
            // Debug.Log($"{_closestHoveringHand} Pos: {_closestHoveringHand.position}");

            //this.rigidbody.position = _closestHoveringHand.position;
    ...
    public void BeginGrasp(List<InteractionController> controllers) {
      _justGrasped = true;
            ...
            Debug.Log($"{gameObject.name} has been grasped");
